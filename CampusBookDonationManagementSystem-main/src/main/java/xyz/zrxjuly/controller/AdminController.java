...
public class AdminController {
	...
	/**查询捐助情况**/
	/** 管理员-查询用户捐赠成功的记录 **/
	@RequestMapping("donationSituation")
	public String donationSituation(Model model) {
		List<assignment> assignmentList=bookService.getAssignment();
		...
		try {
			String childrenListJson = objectMapper.writeValueAsString(childrenList);
			model.addAttribute("childrenListJson", childrenListJson);
			String assignmentListJson = objectMapper.writeValueAsString(assignmentList);
			model.addAttribute("assignmentListJson", assignmentListJson);
		} catch (IOException e) {
			e.printStackTrace();
		}
		model.addAttribute("bookList", bookList);
		model.addAttribute("childrenList", childrenList);
		return "admin/donationSituation";
	}

	/** 管理员-查询用户捐赠成功的记录 **/
	@RequestMapping("donation_historyFind")
	public void donation_historyFind(HttpServletRequest request, HttpServletResponse response) throws SecurityException, IOException, ServletException {
		request.setCharacterEncoding("utf-8");
		String key=request.getParameter("key");
		List<Book> bookList = bookService.getDonationBook1(key);
		request.setAttribute("bookList",bookList);
		request.getRequestDispatcher("/WEB-INF/jsp/admin/book_history.jsp").forward(request,response);
//		model.addAttribute("bookList", bookList);
//		return "admin/book_history";
	}
	...
	@ResponseBody
	public JSONObject addUserInfo(User user, HttpSession session) {
		JSONObject json = new JSONObject();
		User resultUser = userService.selectUserByAccount(user);
		if (resultUser != null){
			json.put("msg", "error");
		}else{
			int addUser = userService.addUserInfo(user);
			if (addUser > 0) {
				json.put("msg", "success");
			} else {
				json.put("msg", "error");
